Winter 2023 CS 32
Homework 5
Time due: 11:00 PM Thursday, March 16
...
binary search tree
Using the simplest binary search tree (BST) insertion algorithm (no balancing), show the tree that results after inserting into the above tree the nodes 80, 62, 75, 15, 35 and 23 in that order. (If you're not skilled with a drawing tool, use a simple text form of the tree. For example, the tree depicted above could be shown as
...
Suppose this tree is being used to represent a set of integers. Write pseudocode to insert a new node into a binary search tree with parent pointers. Since this is for a set, attempting to insert a value already in the tree does nothing. It is your choice what parameters the function takes, whether it is recursive or not, whether it uses an auxiliary function you write, etc. (Hint: You can find binary search tree insertion code on pp. 471-473 of the Carrano book).
...
Turn it in
By Wednesday, March 15, there will be a link on the class webpage that will enable you to turn in this homework. Turn in one zip file that contains your solutions to the homework problems. The zip file must contain one file:
