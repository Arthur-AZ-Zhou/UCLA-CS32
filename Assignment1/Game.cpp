...
bool decodeDirection(char ch, int& dir); //game
bool recommendMove(const Arena& a, int r, int c, int& bestDir); //game
...
string Game::takePlayerTurn()
{
    for (;;)
    {
        cout << "Your move (n/e/s/w/c, type h to see history, or nothing): ";
        string playerMove;
        ...
        if (tolower(playerMove[0] == 'h')) {
            return "history has been called";
        }
...
void Game::play()
{
    ...
    while ( ! player->isDead()  &&  m_arena->rabbitCount() > 0)
    {
        ...
        if (msg == "history has been called") {
